...
export default function StudentAssignments() {
  ...
  const handleFormGroup = async (name) => {
    const group = await createGroup({ courseId: course.id, name, studentId: user.id });
    setGroupModalOpen(false);
    notify(`Created "${group.name}". You're the group leader.`, "success");
  };

  const handleJoinGroup = async (group) => {
    await joinGroup({ groupId: group.id, studentId: user.id });
    setGroupModalOpen(false);
  ...
  const handleAcknowledge = async (assignment) => {
